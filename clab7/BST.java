...
public class BST<Key extends Comparable<Key>> {
    ...
    // Return average depth of the tree
    ...
    private void avgDepthRoot(Node x) {
        if (x != null) {
            avgDepthHelper(x.left);
            avgDepthHelper(x.right);
            avgDepthRoot(x.left);
            avgDepthRoot(x.right);
        }
        return;
    }

    private void avgDepthHelper(Node x) {
        if (x != null) {
            totalDepth += 1;
            avgDepthHelper(x.left);
            avgDepthHelper(x.right);
